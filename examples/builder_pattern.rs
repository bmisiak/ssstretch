...
fn main() {
    ...
    let mut input = [vec![0.0f32; SAMPLE_COUNT], vec![0.0f32; SAMPLE_COUNT]];

    for i in 0..SAMPLE_COUNT {
        let t = i as f32 / SAMPLE_RATE;

        // Left channel - original sine wave
        input[0][i] = (t * FREQUENCY * 2.0 * PI).sin();

        // Right channel - phase-shifted version
        input[1][i] = ((t + 0.25) * FREQUENCY * 2.0 * PI).sin();
    }
    ...
    let mut output = [vec![0.0f32; output_samples], vec![0.0f32; output_samples]];

    // Process audio
    println!(
        "Processing {} input samples into {} output samples...",
